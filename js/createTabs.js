...
function createTabNow(weatherCity) {
  ...
  addCity.src = storage.checkCity(weatherCity.city) ? HEART.FILL : HEART.EMPTY;
...
function createTabForecast(cityName, weatherList) {
  let tab = document.createElement("div");
  ...
  let cards = weatherList.map((element) => createCardForecast(element));
